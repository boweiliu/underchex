...
# Check if file ends in .md and is in .nb_docs_repo/home/
if [[ ! "$FILE_PATH" =~ \.md$ ]] || [[ ! "$FILE_PATH" =~ \.nb_docs_repo/home/ ]]; then
    echo "[DEBUG] $(date '+%H:%M:%S') File does not match criteria (.md in .nb_docs_repo/home/), exiting" >> /tmp/hook-debug.log
    exit 0
fi
echo "[DEBUG] $(date '+%H:%M:%S') File matches criteria (.md in .nb_docs_repo/home/)" >> /tmp/hook-debug.log
