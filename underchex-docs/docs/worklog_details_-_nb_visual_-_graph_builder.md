---
title: Worklog Details   Nb Visual   Graph Builder
---
...
- The script discovers the nb notebook path, parses Markdown files for H1 titles and `[wikilinks](/docs/wikilinks)`, builds node/edge lists, and writes both `graph.json` and a self-contained `index.html`.
...
- `build_graph()` walks all Markdown files, extracts H1 titles, maps titles/paths to ids, and scans `[...](/docs/)` references to build a directed edge set. (`nb-visual/build_nb_graph.py:64`)
