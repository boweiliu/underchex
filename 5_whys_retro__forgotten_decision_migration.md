...
When agent #16.0.0 created [170](/docs/170) (Hex Orientation) to supersede parts of [146](/docs/146) (Hex Coordinate Systems), the doubled-width coordinate decision was not migrated over. The user had to remind the agent about this missing decision.
...
### Why #1: Why was the doubled-width coordinate decision missing from [170](/docs/170)?

**Answer:** The agent focused on the new hex orientation decision (flat-top with odd-q) and didn't review what other decisions from [146](/docs/146) should carry forward.

### Why #2: Why didn't the agent review what decisions from [146](/docs/146) should carry forward?
...
1. **Forgot the "why" behind doubled-width** — The agent remembered "we're using offset coords" but forgot *why* we chose doubled-width in [146](/docs/146): to eliminate the ugly even/odd casework that raw offset coords require. Without remembering the motivation, doubled-width looked like an optional implementation detail rather than the essential fix.
...
2. **No traceable decision chain in the docs** — There's an implicit chain of decisions:
   ...
   ```
   But this chain wasn't explicit in the docs. [146](/docs/146) didn't say "if you're revisiting orientation, also check the doubled-width decision." [170](/docs/170) didn't trace backward to see what depended on offset choice.

3. **Missing backlinks** — [146](/docs/146) should have had forward pointers: "Decisions that depend on this: doubled-width (§Option E)". [170](/docs/170) should have traced: "I'm writing about offset coords → what other decisions in [146](/docs/146) relate to offset? → doubled-width exists because of offset's weakness."
...
**In [146](/docs/146) (the source doc):**
```markdown
## Decision Chain
...
**In [170](/docs/170) (the superseding doc):**
```markdown
## Backlinks Traced

Before writing, I checked [146](/docs/146) for related decisions:
...
## Concrete Examples: What [170](/docs/170) Should Have Looked Like

### Example 1: Header with Supersession Audit Table

**What it had:**
```markdown
**Related**: [146](/docs/146), [166](/docs/166)
...
This doc refines/supersedes parts of [146](/docs/146). Audit:

| From [146](/docs/146)                   | Status      | Notes                                     |
|--------------------------------|-------------|-------------------------------------------|
...
### Example 2: What [146](/docs/146) Should Have Had (Forward Links)

[146](/docs/146) documented doubled-width but didn't flag when it matters:

**What [146](/docs/146) had:**
```markdown
...
**What [146](/docs/146) should have had:**
```markdown
## Recommendation: Option E (Doubled Internal, Offset External)
...
### Example 3: Backlinks Section in [170](/docs/170)

**What [170](/docs/170) was missing:** No explicit trace of what it checked in [146](/docs/146).

**What it should have had:**
```markdown
## Backlinks Traced

Before writing, I reviewed [146](/docs/146) for related decisions:

| Section in [146](/docs/146)     | Relevant? | Action                          |
|------------------------|-----------|----------------------------------|
...
- External coordinates: `(col, row)`
- **Internal storage: doubled-width `_dcol`** (from [146](/docs/146) Option E)

### Carried Forward from [146](/docs/146)
...
- [x] Read [146](/docs/146) completely before writing
...
1. **Source docs** ([146](/docs/146)): Add forward links — "if changing X, also check Y"
2. **Superseding docs** ([170](/docs/170)): Trace backlinks — enumerate what's in source, decide fate of each
3. **Problem-solution pairing**: State the problem next to the solution so readers know *when* it applies

If [146](/docs/146) had said "doubled-width applies whenever you use offset coords (regardless of orientation)" and [170](/docs/170) had a "Backlinks Traced" table, the decision couldn't have been missed.
